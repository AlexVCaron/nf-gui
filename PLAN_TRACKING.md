...
| P0-06, P0-08 | In progress | [Initial phase 0 audit findings](docs/plan/phase-0-audit-findings.md) now map initial features to semantic authorities and highlight key gaps: editor-specific graph commands, unresolved plugin signatures, minimal strict-parser call typing, schema-dialect mismatch, and Monaco version skew. The matrix is still incomplete and phase 0 exit evidence is not yet assembled. | Expand into the full capability matrix and final feature-to-edit-path decision record after the advisory/license follow-up completes. |
