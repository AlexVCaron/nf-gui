...
- [ ] P0-06 — Produce the versioned capability matrix, including unsaved buffers,
...
- [ ] P0-08 — Select compatible Java/server/parser/Nextflow combinations and
...
| P0-03, P0-04 | In progress | [Initial phase 0 audit findings](docs/plan/phase-0-audit-findings.md) record strict-parser, loader, runtime-DAG, typed-parameter, and nf-schema findings with upstream links. Current evidence supports architecture direction but does not yet prove full source-range/module-resolution coverage or plugin-safety boundaries. | Audit parser/compiler source ranges, module resolution, and plugin/init side effects; verify nf-core metadata limits explicitly. |
| P0-05, P0-07 | In progress | [Initial phase 0 audit findings](docs/plan/phase-0-audit-findings.md) now include candidate version pins, licenses, and early advisory notes for Tauri, Monaco, monaco-languageclient, React Flow, Nextflow, and nf-schema. Tauri `2.9.2` release audit surfaced Linux GTK/WebKit dependency warnings; Monaco/LSP bridge compatibility currently points at an unreleased line for `monaco-editor` `0.56.0`. | Finish transitive/provenance/advisory review and choose whether to accept prerelease Monaco bridge dependencies or pin a stable older combination. |
| P0-06, P0-08 | In progress | [Initial phase 0 audit findings](docs/plan/phase-0-audit-findings.md) map initial features to current semantic authorities and identify where custom commands stop and a thin JVM adapter may begin. The matrix is still incomplete and phase 0 exit evidence is not yet assembled. | Expand into the full capability matrix and final feature-to-edit-path decision record before attempting P0-EXIT. |
